...
import boolean_formula_entities.ToolBox;
	...
	private static Expression normalizeBooleanExpression(Expression exp){
		...
		//we only deal with boolean binary expression
			...
			if(ilist==null){
				...
				if(!inexp.isNot()){
					...
					for (int i=0;i<leftlist.size();i++){
						...
						eq.setLeftExpression(leftlist.get(i).value.getExpression());
						eq.setRightExpression(list.get(0));
						
						Expression start;
						start=eq;		
						for (int j=1;j<list.size();j++){
							EqualsTo eqt=new EqualsTo();
							eqt.setLeftExpression(leftlist.get(i).value.getExpression());
							eqt.setRightExpression(list.get(j));
							start=new OrExpression(start,eqt);	
						}
				...
				//if NOT IN
					...
					for (int i=0;i<leftlist.size();i++){
						...
						neq.setLeftExpression(leftlist.get(i).value.getExpression());
						neq.setRightExpression(list.get(0));
						Expression start=neq;		
						for (int j=1;j<list.size();j++){
							NotEqualsTo neqt=new NotEqualsTo();
							neqt.setLeftExpression(leftlist.get(i).value.getExpression());
							neqt.setRightExpression(list.get(j));
							start=new AndExpression(start,neqt);
						}
			...
			if (!between.isNot()){
				...
				AndExpression and=new AndExpression(mte1,mte2);
				...
				for (int i=1;i<leftlist.size();i++){
					...
					and=new AndExpression(mte1,mte2);
					if(leftlist.get(i).conjClause!=null)
				...
				OrExpression or=new OrExpression(mt1,mt2);
				Expression result;
				if(leftlist.get(0).conjClause!=null)
			...
			List<ContingentValue> leftlist=translateExpressionForCombination(left);
			List<ContingentValue> rightlist=translateExpressionForCombination(right);
			Expression result;
			
			LikeExpression newlike=new LikeExpression();
			if(like.isNot())
				newlike.setNot();				
			newlike.setLeftExpression(leftlist.get(0).value.getExpression());
			newlike.setRightExpression(rightlist.get(0).value.getExpression());
			
			Expression start=newlike;
			if(leftlist.get(0).conjClause!=null)
				start=new AndExpression(start,leftlist.get(0).conjClause.reformExpression());
			if(rightlist.get(0).conjClause!=null)
				start=new AndExpression(start,rightlist.get(0).conjClause.reformExpression());
			...
			for (int i=0;i<leftlist.size();i++){
				for(int j=0;j<rightlist.size();j++){
					if(i+j==0)
						...
						newlike=new LikeExpression();
						if(like.isNot())
							newlike.setNot();
						newlike.setLeftExpression(leftlist.get(i).value.getExpression());
						newlike.setRightExpression(rightlist.get(j).value.getExpression());
						start=newlike;
						if(leftlist.get(i).conjClause!=null)
							start=new AndExpression(start,leftlist.get(i).conjClause.reformExpression());
						if(rightlist.get(j).conjClause!=null)
		...
		if (swexp!=null){
			...
			for (ContingentValue sw: swvalues){
				...
				for (WhenClause wc: wclist){
					...
					EqualsTo eq=new EqualsTo();
					eq.setLeftExpression(sw.value.getExpression());
					eq.setRightExpression(when);
					BooleanLiteral bl;
